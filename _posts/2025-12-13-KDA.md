...
### Simple Derivations of Linear Attention

Turning Transformers into **RNN-like Linear-Time models**. The transition relies entirely on the **associativity of matrix multiplication**.

#### 1. Dropping the Softmax
In standard Softmax attention, the denominator $\sum \exp(\dots)$ and the non-linear $\exp$ prevent us from separating the query $q_t$ from the keys and values. By removing the softmax (or replacing it with a kernel feature map $\phi(x)$), we get:

$$o_t = \sum_{j=1}^{t} (q_t^\top k_j) v_j$$

#### 2. The Reordering (Associative Property)
The term $(q_t^\top k_j)$ is a scalar. In linear algebra, for a scalar $c$ and a vector $v$, $c v = v c$. Furthermore, $q_t^\top k_j = k_j^\top q_t$. Therefore:

$$o_t = \sum_{j=1}^{t} v_j (k_j^\top q_t)$$

Because $q_t$ does not depend on the summation index $j$, we can factor it out to the right:

$$o_t = \left( \sum_{j=1}^{t} v_j k_j^\top \right) q_t$$

#### 3. Defining the Running State ($S_t$)
The term in the parentheses is a summation of outer products. Each $v_j k_j^\top$ results in a $d \times d$ matrix. We define this cumulative sum as our "memory" or "state" matrix $S_t$:

$$S_t = \sum_{j=1}^{t} v_j k_j^\top$$

This gives us the **Recursive Update Rule**:
$$S_t = S_{t-1} + v_t k_t^\top$$

#### 4. Why this Solves the $O(n^2)$ Bottleneck
The difference in computational complexity during inference is significant:

| Method | Equation | Complexity (per step) | Complexity (Total $T$ steps) |
| :--- | :--- | :--- | :--- |
| **Standard** | $o_t = \sum (q_t^\top k_j) v_j$ | $O(t \cdot d)$ | **$O(T^2 d)$** |
| **Linear** | $o_t = S_t q_t$ | $O(d^2)$ | **$O(T d^2)$** |

* **Standard Attention:** At step $t$, you must perform $t$ dot products. The cost grows with the sequence length.
* **Linear Attention:** At any step $t$, you perform one outer product to update $S_t$ ($d \times d$) and one matrix-vector multiply to get $o_t$. The cost is constant relative to $t$.

#### 5. From "Naïve" to "Gated/Norm"
The naïve version $S_t = S_{t-1} + v_t k_t^\top$ can be unstable because the values in $S_t$ can grow unbounded. To fix this, practical variants use:

1.  **Normalization:** Dividing the output by a running sum of keys to mimic the softmax denominator.
2.  **Gating (Delta Rule):** Instead of just adding $v_t k_t^\top$, we use the Delta Rule: 
    $$S_t = S_{t-1} + \beta_t(v_t - S_{t-1}k_t)k_t^\top$$
    This allows the model to "overwrite" memory rather than just accumulating it.

#### Summary of the Training Reordering
In training, where all $Q, K, V$ are available, the reordering is:
$$(QK^\top)V \implies Q(K^\top V)$$

* $(QK^\top)V$: $(N \times N) \times (N \times d)$
* $Q(K^\top V)$: $(N \times d) \times (d \times d)$

When $N \gg d$ (long context), the second form is significantly more efficient.
